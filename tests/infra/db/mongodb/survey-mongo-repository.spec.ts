...
describe('Survey Mongo Repository', () => {
  ...
  describe('loadAnswers()', () => {
    test('Should load answers on success', async () => {
      ...
      expect(answers[0]).toBe(addSurveyParams.answers[0].answer)
    })

    test('Should return [] if answers not exists', async () => {
      const sut = makeSut()
      const id = new ObjectId()
      const answers = await sut.loadAnswers(id.toHexString())
  ...
  describe('checkById()', () => {
    ...
    test('Should return false if survey not exits', async () => {
      const sut = makeSut()
      const id = new ObjectId()
      const surveyExists = await sut.checkById(id.toHexString())
